...
if(is_array($query) && count($query) == 3 && in_array($query['type'], array(WECHATS, WXAPP)) && !empty($query['appid']) && !empty($query['secret'])) {
	...
	$account_info = pdo_fetch('SELECT w.* FROM ' . tablename($table_name) . ' AS w LEFT JOIN ' . tablename('account') . ' AS a ON w.uniacid = a.uniacid WHERE w.key = :wxkey AND w.secret = :wxsecret AND a.isdeleted = :isdeleted', array(':wxkey' => $query['appid'], ':wxsecret' => $query['secret'], ':isdeleted' => STATUS_OFF));
