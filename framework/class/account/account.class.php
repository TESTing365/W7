...
class WeAccount extends ArrayObject {
	...
	public function __construct($uniaccount = array()) {
		$this->uniacid = empty($uniaccount['uniacid']) ? 0 : $uniaccount['uniacid'];
	...
	protected function fetchCreateTime() {
		global $_W;
		//私有类创始人账号在外层没有操作权限
		if ($_W['uid'] == $this->account['create_uid'] || !empty($_W['isadmin'])) {
	...
	protected function fetchGroupLevel() {
		...
		return empty($this->setting['grouplevel']) ? '' : $this->setting['grouplevel'];
	}
