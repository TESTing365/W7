...
class Mobile extends OAuth2Client {
	...
	public function user() {
		global $_GPC, $_W;
		$mobile = safe_gpc_string($_GPC['username']);
		$password = safe_check_password($_GPC['password']);
		if (is_error($password)) {
			return error(-1, $password['message']);
		}
	...
	public function validateMobile() {
		...
		if (empty($mobile)) {
			return error(-1, '手机号不能为空或手机号格式不正确');
		}
	...
	public function paramValidate() {
		...
		$image_code = safe_gpc_string($_GPC['imagecode']);
		$sms_code = safe_gpc_string($_GPC['smscode']);
		$need_checkcaptcha = true;
		if (isset($_GPC['need_checkcaptcha']) && empty($_GPC['need_checkcaptcha'])) {
			$need_checkcaptcha = false;
		}
