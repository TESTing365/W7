...
function cache_build_frame_menu() {
	...
	if (!empty($system_menu) && is_array($system_menu)) {
		...
		foreach ($system_menu as $menu_name => $menu) {
			...
			if (!empty($_W['role']) && $_W['role'] == ACCOUNT_MANAGE_NAME_EXPIRED && $menu_name != 'system') {
			...
			foreach ($menu['section'] as $section_name => $section) {
				...
				foreach ($section['menu'] as $permission_name => $sub_menu) {
					...
					$sub_menu_db = empty($system_menu_db[$sub_menu['permission_name']]) ? '' : $system_menu_db[$sub_menu['permission_name']];
					$is_display = 1;
					...
					if ($is_display && isset($sub_menu['is_display'])) {
						...
						if (empty($_W['isw7_request']) && is_array($sub_menu['is_display']) && !empty($account['type']) && !in_array($account['type'], $sub_menu['is_display'])) {
							$is_display = 0;
						}
					}
					$system_menu[$menu_name]['section'][$section_name]['menu'][$permission_name] = array(
						...
						'permission_display' => empty($sub_menu['is_display']) ? '' : $sub_menu['is_display'],
						'is_display' => $is_display,
						'title' => !empty($sub_menu_db['title']) ? $sub_menu_db['title'] : $sub_menu['title'],
						'url' => $sub_menu['url'],
						'permission_name' => $sub_menu['permission_name'],
						'icon' => empty($sub_menu['icon']) ? '' : $sub_menu['icon'],
						'displayorder' => !empty($sub_menu_db['displayorder']) ? $sub_menu_db['displayorder'] : $displayorder,
						'id' => empty($sub_menu['id']) ? '' : $sub_menu['id'],
						'sub_permission' => empty($sub_menu['sub_permission']) ? '' : $sub_menu['sub_permission'],
					);
					$displayorder--;
...
function cache_updatecache() {
	...
	$cloud_w7_request_token_cache = cache_read(cache_system_key('cloud_w7_request_token'));
	//无论是哪种缓存方式，更新缓存时强制删除数据库中的值
	pdo_delete('core_cache');
	cache_clean();
	cache_write(cache_system_key('account_ticket'), $account_ticket_cache);
	cache_write(cache_system_key('cloud_w7_request_token'), $cloud_w7_request_token_cache);
