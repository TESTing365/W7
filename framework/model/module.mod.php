...
function module_system() {
	return array(
		...
		'custom', 'chats', 'paycenter', 'keyword', 'special', 'welcome', 'default', 'apply', 'reply', 'core', 'wxapp'
	);
}
...
function module_uninstall_total($type) {
	...
	if (!empty($_W['config']['setting']['local_dev'])) {
		$total = table('modules_cloud')->where('install_status', MODULE_LOCAL_UNINSTALL)->getcolumn('count(*)');
	} else {
		if ($type == 'all') {
