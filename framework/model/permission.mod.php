...
function permission_account_user($type = 'system') {
	...
	$user_permission = empty($user_permission['permission']) ? array() : $user_permission['permission'];
	if (empty($user_permission)) {
		$user_permission = array('account*', 'wxapp*', 'phoneapp*');
	}
	$permission_append = frames_menu_append();
	//目前只有系统管理才有预设权限，公众号权限走数据库
	if (!empty($_W['role']) && !empty($permission_append[$_W['role']])) {
...
function permission_account_user_menu($uid, $uniacid, $type) {
	...
	if ($type == 'modules') {
		...
		if (in_array($type, $module) && !empty($user_menu_permission['modules']) && $user_menu_permission['modules']['permission'] == 'all') {
			return array('all');
		}
...
function permission_check_account_user($permission_name, $show_message = true, $action = '') {
	global $_W, $_GPC, $acl;
	load()->model('module');
	$see_more_info = empty($acl['see_more_info']) ? array() : $acl['see_more_info'];
	...
	if (strpos($permission_name, 'see_') === 0) {
		...
		if (in_array(FRAME, array('system', 'site', 'myself', 'user_manage', 'permission'))) {
			...
			if (!empty($see_more_info[$_W['role']]) && is_array($see_more_info[$_W['role']])) {
				$can_see_more = in_array($permission_name, $see_more_info[$_W['role']]) ? true : false;
			}
		}
		/*用户有上级则不能修改(owner_uid)start*/
		$uniacid = empty($_GPC['uniacid']) ? 0 : safe_gpc_int($_GPC['uniacid']);
...
function permission_user_account_num($uid = 0) {
	...
	foreach ($account_all_type_sign as $sign) {
		$maxsign = 'max' . $sign;
		$create_group_info_all[$maxsign] = empty($create_group_info_all[$maxsign]) ? 0 : $create_group_info_all[$maxsign];
		$extra_limits_info[$maxsign] = empty($extra_limits_info[$maxsign]) ? 0 : $extra_limits_info[$maxsign];
		...
		$sign_num = $sign . '_num';
		$group[$maxsign] = empty($group[$maxsign]) ? 0 : $group[$maxsign];
		$limit[$sign] = max((intval($group[$maxsign]) + $extra[$sign] - $group_num[$sign_num]), 0);
		$group_vice[$maxsign] = empty($group_vice[$maxsign]) ? 0 : $group_vice[$maxsign];
	...
	$data = array(
		...
		'vice_group_name' => empty($group_vice['name']) ? '' : $group_vice['name'],
	...
	foreach ($account_all_type_sign as $sign) {
		...
		//用户组剩余创建数量
		$data['usergroup_' . $sign . '_limit'] = max($group[$maxsign] - $group_num[$sign_num], 0);
		//最大创建数量
		$data[$maxsign] = $group[$maxsign] + $extra[$sign];
		//已创建的数量
	...
	if (empty($user_own_module_permission)) {
		if ($account->supportVersion) {
			...
			if (!empty($version_all)) {
				foreach ($version_all as $version) {
			...
			foreach ($version_modules as $version_module) {
				pdo_insert('users_permission', array('uniacid' => $uniacid, 'uid' => $uid, 'type' => $version_module, 'permission' => 'all'));
			}
		} else {
			pdo_insert('users_permission', array('uniacid' => $uniacid, 'uid' => $uid, 'type' => 'modules', 'permission' => 'all'));
		}
	}
	return true;
}
