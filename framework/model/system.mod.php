...
function system_check_statcode($statcode) {
	...
	foreach ($allowed_stats as $key => $item) {
		...
		return safe_gpc_html(htmlspecialchars_decode($statcode, ENT_QUOTES));
	}
}
...
function system_check_items() {
	return array(
		'mbstring' => array(
			...
			'handle' => 'https://market.w7.cc/IndependentEngine'
		),
		'mcrypt' => array(
			'operate' => 'system_check_php_ext',
...
function system_check_site_https() {
	global $_W;
	return 0 === strpos($_W['setting']['site']['url'], 'https');
}
/**
 * 检测站点 php 拓展是否开启
 * @param $extension
...
function system_scrap_file() {
	$scrap_file = array(
		...
		'/web/source/system/template.ctrl.php',
