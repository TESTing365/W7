...
function file_remote_upload($filename, $auto_delete_local = true) {
	...
	} elseif ($_W['setting']['remote']['type'] == ATTACH_TCB) {
		$upload_link = ihttp_request('https://api.weixin.qq.com/tcb/uploadfile', json_encode(array('env' => getenv('CBR_ENV_ID'), 'path' => $filename)));
		$upload_link = json_decode($upload_link['content'], true);
		if (0 != $upload_link['errcode']) {
			return error(1, '上传云托管时发生错误，错误码：' . $upload_link['errcode'] . '，详情：' . $upload_link['errmsg']);
		}
		$response = ihttp_request($upload_link['url'], array(
			'key' => $filename,
			'Signature' => $upload_link['authorization'],
			'x-cos-security-token' => $upload_link['token'],
			'x-cos-meta-fileid' => $upload_link['cos_file_id'],
			'file' => new \CURLFile(ATTACHMENT_ROOT . $filename),
		), array('Content-Type' => 'multipart/form-data'));
		$result = json_decode($response['content'], true);
		if (204 == $response['code'] && empty($result)) {
			if ($auto_delete_local) {
				file_delete($filename);
			}
			//返回file_id并入库，用于之后对文件进行删除操作
			return $upload_link['file_id'];
		} else {
			return error(1, '上传云托管时发生错误，错误码：' . $result['errocde'] . '，详情：' . $result['errmsg']);
		}
	}
	return true;
}
...
function file_remote_delete($file) {
	...
	if (!empty($_W['setting']['remote']['type'])) {
		switch ($_W['setting']['remote']['type']) {
			case ATTACH_FTP:
				...
				if (true === $ftp->connect()) {
					if (!$ftp->delete_file($file)) {
						$result = error(1, '删除附件失败，请检查配置并重新删除');
					}
				} else {
					$result = error(1, '删除附件失败，请检查配置并重新删除');
				}
			...
			case ATTACH_TCB:
				global $media;
				$api_delete = ihttp_request('https://api.weixin.qq.com/tcb/batchdeletefile', json_encode(array('env' => getenv('CBR_ENV_ID'), 'fileid_list' => array($media['tcb_file_id']))));
				$api_delete = json_decode($api_delete['content'], true);
				if (0 != $api_delete['errcode']) {
					return error(1, '删除失败，错误码：' . $api_delete['errcode'] . '，详情：' . $api_delete['errmsg']);
				}
				break;
		}
	}
