...
function current_operate_is_controller() {
	...
	if (in_array(FRAME, array('welcome', 'module_manage', 'user_manage', 'permission', 'system', 'site', 'account_manage'))) {
		$result = 1;
	}
	$_GPC['m'] = empty($_GPC['m']) ? '' : $_GPC['m'];
	$_GPC['module_name'] = empty($_GPC['module_name']) ? '' : $_GPC['module_name'];
...
function buildframes($framename = '') {
	...
	if (!empty($user_permission)) {
		foreach ($frames as $nav_id => $section) {
			...
			foreach ($section['section'] as $section_id => $secion) {
				if ('account' == $nav_id) {
					if (!empty($module_permission) && in_array('account*', $user_permission) && 'platform_module' != $section_id && ACCOUNT_MANAGE_NAME_OWNER != permission_account_user_role($_W['uid'], $_W['uniacid'])) {
			...
			if (!empty($_W['role']) && ACCOUNT_MANAGE_NAME_EXPIRED == $_W['role'] && 'system' != $nav_id) {
				$menu['is_display'] = 0;
			}
		}
	} else {
					...
					if (!in_array($module['type'], $sysmodules) && $modules[$module['type']][MODULE_SUPPORT_ACCOUNT_NAME] == 2) {
						...
						if (!empty($module)) {
							$frames[FRAME]['section']['platform_module']['menu']['platform_' . $module['name']] = array(
								'title' => $module['title'],
								'icon' => $module['logo'],
								'url' => url('home/welcome/ext', array('module_name' => $module['name'])), //兼容客户端控制台,统一进应用首页(原来是account_ext,可以直接进应用)
								'is_display' => 1,
							);
						}
					}
			...
			if (!empty($account_module)) {
				...
				foreach ($new_modules as $module) {
					...
					$frames[FRAME]['section']['platform_module']['menu']['platform_' . $module['name']] = array(
						'title' => $module['title'],
						'icon' => $module['logo'],
						'url' => url('home/welcome/ext', array('module_name' => $module['name'])),
						'is_display' => 1,
					);
					++$i;
				}
	...
	if ((!empty($modulename) || !empty($eid)) && !in_array($modulename, module_system())) {
		...
		if ($module['permissions'] && in_array($account_user_role, array(ACCOUNT_MANAGE_NAME_OWNER, ACCOUNT_MANAGE_NAME_VICE_FOUNDER, ACCOUNT_MANAGE_NAME_FOUNDER)) && !defined('SYSTEM_WELCOME_MODULE')) {
		...
		if (!empty($entries['home']) && !empty($_W['account']) && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) {
		...
		if (!empty($entries['profile']) && !empty($_W['account']) && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) {
		...
		if (!empty($entries['shortcut']) && !empty($_W['account']) && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) {
			...
			foreach ($wxapp_version['modules'] as $module) {
				...
				if (!empty($module) && (isset($module['recycle_info']) ? (empty($module['recycle_info']['wxapp_support']) ? true : false) : true)) {
					$frames['wxapp']['section']['platform_module']['menu']['module_menu' . $module['mid']] = array(
						'icon' => $module['logo'],
						'title' => $module['title'],
						'url' => url('home/welcome/ext', array('module_name' => $module['name'], 'version_id' => $version_id)),
						'is_display' => 1,
					);
				}
			}
...
function frames_top_menu($frames) {
	...
	$_W['highest_role'] = empty($_W['highest_role']) ? '' : $_W['highest_role'];
	foreach ($frames as $menuid => $menu) {
		if ((!empty($menu['founder']) || in_array($menuid, array('module_manage', 'site'))) && empty($_W['isadmin']) ||
			ACCOUNT_MANAGE_NAME_CLERK == $_W['highest_role'] && in_array($menuid, array('account', 'wxapp', 'system', 'platform', 'welcome', 'account_manage')) ||
			!$is_vice_founder && empty($_W['isadmin']) && in_array($menuid, array('user_manage', 'permission')) ||
			'myself' == $menuid && !empty($_W['isadmin']) ||
			!$menu['is_display']) {
			continue;
		}
