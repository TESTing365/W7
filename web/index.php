...
if (empty($_W['isadmin']) && (!empty($_W['role']) && $_W['role'] == ACCOUNT_MANAGE_NAME_EXPIRED || !empty($_W['highest_role']) && $_W['highest_role'] == ACCOUNT_MANAGE_NAME_EXPIRED)) {
	$_W['isajax'] ? iajax(-1, '您的账号已过期，请联系管理员处理！') : itoast('您的账号已过期，请联系管理员处理！');
}
...
// navs end;
if ((!empty($_W['setting']['copyright']['status']) && $_W['setting']['copyright']['status'] == 1) && empty($_W['isfounder']) && 'cloud' != $controller && 'utility' != $controller && 'account' != $controller) {
	$_W['siteclose'] = true;
	...
	itoast('站点已关闭，关闭原因：' . $_W['setting']['copyright']['reason'], url('account/welcome'), 'info');
}
...
if (ACCOUNT_MANAGE_NAME_FOUNDER != $_W['highest_role']) {
	if (empty($_W['uniacid']) && in_array(FRAME, array('account', 'wxapp')) && !$_GPC['system_welcome']) {
	...
	foreach ($frames['section'] as &$frame) {
		...
		foreach ($frame['menu'] as $key => &$menu) {
			if (defined('IN_MODULE') && !empty($menu['multilevel'])) {
				...
				if (!empty($get['do']) && false !== strpos($get['do'], 'post') && !in_array($key, array('platform_menu', 'platform_masstask'))) {
					$_W['page']['title'] = '';
					continue;
				}
				unset($urls['w7i']);
