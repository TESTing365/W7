...
!defined('FRAME') && define('FRAME', 'system');
load()->model('system');
	...
	if (!empty($version_exist)) {
		...
		if (!empty($version_lists)) {
			foreach ($version_lists as &$row) {
				$row['version_id'] = empty($row['version_id']) ? 0 : $row['version_id'];
				...
				if (empty($row['last_modules'])) {
					$row['last_modules'] = empty($row['module']) ? '' : $row['module'];
				}
			}
			unset($row);
		}
		if (!empty($version_exist['version']['type']) && WXAPP_CREATE_DEFAULT < $version_exist['version']['type']) {
