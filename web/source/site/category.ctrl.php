...
if ($do == 'display') {
	...
	$category['style'] = empty($category['styleid']) ? array() : $styles[$category['styleid']];
	...
	if (checksubmit('submit')) {
		...
		if ($data['icontype'] == 1) {
			...
			$data['css'] = serialize(array(
				'icon' => array(
					'font-size' => safe_gpc_string($_GPC['icon']['size']),
					'color' => safe_gpc_string($_GPC['icon']['color'], '', 'color'),
					'width' => safe_gpc_string($_GPC['icon']['size']),
					'icon' => empty($_GPC['icon']['icon']) ? 'fa fa-external-link' : safe_gpc_string($_GPC['icon']['icon']),
				),
			));
		} else {
			$data['css'] = '';
			$data['icon'] = safe_gpc_string($_GPC['iconfile']);
		}
		...
		if ($isnav) {
			...
			if ($data['icontype'] == 1) {
				...
				$nav['css'] = serialize(array(
					'icon' => array(
						'font-size' => intval($_GPC['icon']['size']),
						'color' => safe_gpc_string($_GPC['icon']['color'], '', 'color'),
						'width' => intval($_GPC['icon']['size']),
						'icon' => empty($_GPC['icon']['icon']) ? 'fa fa-external-link' : safe_gpc_string($_GPC['icon']['icon']),
					),
					'name' => array(
						'color' => safe_gpc_string($_GPC['icon']['color']),
					),
				));
			} else {
				$nav['css'] = '';
				$nav['icon'] = safe_gpc_string($_GPC['iconfile']);
			}
			if ($category['nid']) {
