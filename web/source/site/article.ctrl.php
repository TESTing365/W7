...
if ($do == 'display') {
	...
	$pcate = empty($_GPC['pcate']) ? 0 : intval($_GPC['pcate']);
	$ccate = empty($_GPC['ccate']) ? 0 : intval($_GPC['ccate']);
	if (!empty($id)) {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($_GPC['thumb'])) {
			if (file_is_image($_GPC['thumb'])) {
				$data['thumb'] = safe_gpc_url($_GPC['thumb']);
			}
		...
		$keyword = str_replace('，', ',', safe_gpc_string($_GPC['keyword']));
		$keyword = empty($keyword) ? array() : explode(',', $keyword);
		if (!empty($keyword)) {
			...
			if (!empty($result)) {
				...
				foreach ($rules as $rule) {
					...
					$ruleurl .= "<a href='./index.php?c=platform&a=reply&do=post&rid=" . $rule['id'] . "'><strong class='text-danger'>" . $name . "</strong></a>&nbsp;";
				}
				itoast('该关键字已存在于 ' . $ruleurl . ' 规则中.', '', '');
			}
			$rule['uniacid'] = $_W['uniacid'];
			$rule['name'] = '文章：' . safe_gpc_string($_GPC['title']) . ' 触发规则';
			$rule['module'] = 'news';
			$rule['status'] = 1;
			$keywords = array();
			...
			$reply['title'] = safe_gpc_string($_GPC['title']);
			$reply['description'] = safe_gpc_string($_GPC['description']);
