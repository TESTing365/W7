...
if ('cardsn' == $do) {
	$uid = intval($_GPC['uid']);
	$cardsn = safe_gpc_string($_GPC['cardsn']);
	$type = safe_gpc_string($_GPC['type']);
...
if ('credit' == $do) {
	...
	$status = mc_credit_update($uid, $type, $num, array($_W['user']['uid'], safe_gpc_string($_GPC['remark']), 'system', $_W['user']['clerk_id'], $_W['user']['store_id'], $_W['user']['clerk_type']));
	if (is_error($status)) {
...
if ('card' == $do) {
	...
	$record = array(
		...
		'uid' => $uid,
		'cid' => $card['id'],
		'cardsn' => safe_gpc_string($_GPC['username']),
		'status' => '1',
		'createtime' => TIMESTAMP,
		'endtime' => TIMESTAMP,
	);
