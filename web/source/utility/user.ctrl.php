...
if ('browser' == $do) {
	$mode = in_array($_GPC['mode'], array('invisible', 'visible')) ? safe_gpc_string($_GPC['mode']) : 'visible';

	$callback = safe_gpc_string($_GPC['callback']);

	$uids = safe_gpc_string($_GPC['uids']);
	$uidArr = array();
	if (empty($uids)) {
		$uids = '';
	} else {
	...
	if (user_is_vice_founder()) {
		...
		if (!empty($founder_users)) {
			$founder_users = array_map(function ($item) {
				return intval($item);
			}, array_keys($founder_users));
