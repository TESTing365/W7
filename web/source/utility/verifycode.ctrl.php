...
$custom_sign = empty($_GPC['custom_sign']) ? '' : safe_gpc_string($_GPC['custom_sign']);
...
$receiver = safe_gpc_string($_GPC['receiver']);
if (empty($receiver)) {
	iajax(-1, '请输入邮箱或手机号');
...
if ('send_code' == $do) {
	...
	if (!empty($row)) {
		$imagecode = empty($_GPC['imagecode']) ? '' : intval($_GPC['imagecode']);
