...
$_W['page']['title'] = '';
$callback = empty($_GPC['callback']) ? '' : safe_gpc_string($_GPC['callback']);
...
if ($do == 'articlelist') {
	...
	if (!empty($_GPC['keyword'])) {
		...
		$param = array(':uniacid' => $_W['uniacid'], ':title' => '%' . safe_gpc_string($_GPC['keyword']) . '%');
	} else {
	...
	if (!empty($result['list'])) {
		foreach ($result['list'] as $k => &$v) {
			...
			$v['name'] = empty($v['name']) ? '' : cutstr($v['name'], 10);
		}
		unset($v);
