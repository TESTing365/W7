...
if (2 == $_W['setting']['site']['install_type']) {
	$setting['folder'] = $_W['setting']['site']['key'] . '/' . $setting['folder'];
}
if ('fetch' == $do) {
...
if ('upload' == $do) {
	if (empty($_W['setting']['remote']['type']) && 2 == $_W['setting']['site']['install_type']) {
		iajax(-1, '云托管安装的系统需要配置远程附件，否则环境重启后无法保存之前上传的图片。<br>您可以：1、在系统配置远程附件；2、微信云托管里配置 <a class="color-default" href="https://wiki.w7.com/document/35/5841">云调用</a>');
	}
...
if ('fetch' == $do || 'upload' == $do) {
	...
	pdo_insert('core_attachment', array(
		...
		'tcb_file_id' => !empty($remotestatus) && !empty($_W['setting']['remote']['type']) && ATTACH_TCB == $_W['setting']['remote']['type'] ? $remotestatus : '',
	));
	if ($_W['isw7_request']) {
		iajax(0, '上传成功！');
	}
