...
if ('edit' == $do) {
	...
	$permissions = pdo_fetchall('SELECT id, uid, role FROM ' . tablename('uni_account_users') . " WHERE uniacid = :uniacid ORDER BY uid ASC, role DESC", array(':uniacid' => $uniacid), 'uid');
	if (!empty($permissions)) {
		$member = table('users')->select(['uid', 'username'])->where('uid IN', array_keys($permissions))->getall('uid');
		if (!empty($member)) {
...
if ('set_manager' == $do) {
	...
	if (!empty($user)) {
		...
		//添加/修改公众号主管理员时执行数量判断
		if (ACCOUNT_MANAGE_TYPE_OWNER == $addtype && !in_array($user['uid'], $founders)) {
		...
		if (empty($exists)) {
			...
			if ((empty($permissions) || empty($modules_str)) && !empty($exists)) {
				if (ACCOUNT_MANAGE_NAME_OPERATOR == $exists['role']) {
					$role_name = '操作员';
...
if ('set_permission' == $do || 'save_permission' == $do) {
	...
	if ($account->supportVersion) {
		...
		foreach ($version_modules as $version_module) {
			...
			if (!empty($module[$version_module]['plugin_list'])) {
				foreach ($module[$version_module]['plugin_list'] as $plugin_module) {
					$module[$plugin_module] = module_fetch($plugin_module);
				}
			}
		}
	} else {
