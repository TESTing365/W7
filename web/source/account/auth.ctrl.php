...
if ('forward' == $do) {
	...
	if (is_error($auth_info)) {
		itoast('授权登录新建公众号失败：' . $auth_info['message'], $redirect, 'error');
	}
	...
	if (is_error($account_info)) {
		itoast('授权登录新建公众号失败：' . $account_info['message'], $redirect, 'error');
	}
	if (!empty($_GPC['test'])) {
	...
	if (!$insert_result) {
		itoast('授权登录新建公众号失败，请重试', $redirect, 'error');
	}
	...
	itoast('授权登录成功', $redirect, 'success');
}
if ('confirm' == $do) {
	...
	if (empty($user_accounts) || !in_array($uniacid, $user_accounts)) {
		itoast('账号或用户信息错误!', $redirect, 'error');
	}
	...
	itoast('更改公众号授权接入成功', $redirect, 'success');
}
if ('ticket' == $do) {
...
if ('test' == $do) {
	...
	$auth_type_info = array(
		'account' => ACCOUNT_PLATFORM_API_LOGIN_ACCOUNT,
		'wxapp' => ACCOUNT_PLATFORM_API_LOGIN_WXAPP,
	);
	$auth_type = $auth_type_info[$type] ? $auth_type_info[$type] : '';
	$authurl = $account_platform->getAuthLoginUrl($auth_type, true);
