...
if ('display' == $do) {
	...
	$account_type = empty($_GPC['account_type']) ? '' : safe_gpc_string($_GPC['account_type']);
...
if ('recover' == $do || 'delete' == $do) {
	...
	if (!empty($uniacid)) {
		...
		$uniacids = empty($_GPC['uniacids']) ? array() : safe_gpc_array($_GPC['uniacids']);
	}
	if (empty($uniacids)) {
		if ($_W['isajax']) {
			iajax(-1, '缺少必要参数！');
...
if ('recover' == $do) {
	...
	foreach ($uniacids as $uniacid) {
		...
		if (!(!empty($account_info[$sign_limit]) && (!empty($account_info[$founder_sign_limit]) && $_W['user']['owner_uid'] || empty($_W['user']['owner_uid'])) || $_W['isadmin'])) {
			if ($_W['isajax']) {
		...
		if (!empty($appid)) {
			...
			if (!empty($hasAppid)) {
				if ($_W['isajax']) {
					iajax(-1, "平台 {$account['name']} 的 {$hasAppid['key_title']} 已被其他平台使用, 请停用{$hasAppid['type_title']}[ {$hasAppid['name']} ]后再恢复.");
				}
				itoast("平台 {$account['name']} 的 {$hasAppid['key_title']}已被其他平台使用, 请停用{$hasAppid['type_title']}[ {$hasAppid['name']} ]后再恢复.", referer(), 'error');
			}
		}
