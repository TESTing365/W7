...
if ('get_authurl' == $do) {
	if (empty($_W['setting']['platform']['authstate'])) {
		...
		iajax(0, $_W['siteroot'] . 'web/index.php?c=account&a=auth&do=test&type=' . $sign);
	}
}
...
if ('save_account' == $do || 'check_params' == $do) {
	...
	$post['description'] = empty($_GPC['description']) ? '' : safe_gpc_string($_GPC['description']);
	$post['owner_uid'] = empty($_GPC['owner_uid']) ? 0 : intval($_GPC['owner_uid']);
	$post['version'] = empty($_GPC['version']) ? '' : safe_gpc_string(trim($_GPC['version']));

	if (empty($post['step']) || 'base_info' == $post['step']) {
		if (empty($post['name'])) {
			iajax(-1, $sign_title . '名称不能为空');
		}
		$check_uniacname = table('uni_account')->select('uniacid')->where('name', $post['name'])->get();
		if (!empty($check_uniacname)) {
			iajax(-1, "名称：“'{$post['name']}'”已经存在");
		}
		//验证appid唯一性
		if (in_array($sign, array(ACCOUNT_TYPE_SIGN))) {
			$appid = empty($_GPC['key']) ? '' : safe_gpc_string($_GPC['key']);
		} else {
			$appid = empty($_GPC['appid']) ? '' : safe_gpc_string($_GPC['appid']);
		}
		if (!empty($appid)) {
...
if ('save_account' == $do) {
	...
	if (!empty($_GPC['headimg'])) {
		...
		if (file_is_image($headimg)) {
			$attachment_url[] = str_replace($_W['attachurl'], '', $headimg);
		}
	}
	if (!empty($_GPC['qrcode'])) {
		$qrcode = safe_gpc_path($_GPC['qrcode']);
		if (file_is_image($qrcode)) {
			$attachment_url[] = str_replace($_W['attachurl'], '', $qrcode);
		}
	}
	// step1 基础信息和权限数据处理
	if (in_array($sign, array(ACCOUNT_TYPE_SIGN, WEBAPP_TYPE_SIGN, PHONEAPP_TYPE_SIGN))) {
		...
		if (ACCOUNT_TYPE_SIGN == $sign) {
			$account_data['account'] = empty($_GPC['account']) ? '' : safe_gpc_string(trim($_GPC['account']));
		}
		if (ACCOUNT_TYPE_SIGN == $sign) {
			$account_data['original'] = empty($_GPC['original']) ? '' : safe_gpc_string(trim($_GPC['original']));
			$account_data['level'] = empty($_GPC['level']) ? '' : intval($_GPC['level']);
			$account_data['key'] = empty($_GPC['key']) ? '' : safe_gpc_string(trim($_GPC['key']));
			$account_data['secret'] = empty($_GPC['secret']) ? '' : safe_gpc_string(trim($_GPC['secret']));
		}
	...
	if (in_array($sign, array(WXAPP_TYPE_SIGN, ALIAPP_TYPE_SIGN, BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) {
		...
		if (WXAPP_TYPE_SIGN == $sign) {
			$miniapp_data['original'] = empty($_GPC['original']) ? '' : safe_gpc_string($_GPC['original']);
	...
	if (in_array($sign, array(PHONEAPP_TYPE_SIGN, WXAPP_TYPE_SIGN, ALIAPP_TYPE_SIGN, BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) {
		...
		//打包模块
		$module = empty($_GPC['version_module']) ? '' : module_fetch(safe_gpc_string($_GPC['version_module']));
		if (!empty($module)) {
