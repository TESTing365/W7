...
if ('list' == $do) {
	...
	$type = in_array($_GPC['type'], array_keys($account_all_type_sign)) ? safe_gpc_string($_GPC['type']) : 'all';
	...
	$search_role = !empty($_GPC['role']) && in_array($_GPC['role'], array('owner', 'manager', 'operator')) ? safe_gpc_string($_GPC['role']) : '';
	if ($search_role) {
	...
	if ('display' == $do) {
		...
		$orderby = safe_gpc_string($_GPC['orderby']) == 'initials' ? 'initials' : 'createtime';
		switch($orderby) {
			case 'createtime':
				$table->accountUniacidOrder();
				break;
	...
	if (!empty($list)) {
		...
		foreach ($list as $k => &$account) {
			...
			if (!empty($account_user_roles[$account['uniacid']]['role']) && !in_array($account_user_roles[$account['uniacid']]['role'], array(ACCOUNT_MANAGE_NAME_OWNER, ACCOUNT_MANAGE_NAME_MANAGER)) && !$_W['isfounder']) {
			...
			$account['level'] = empty($account['type_sign']) || empty($account['level']) || empty($account_all_type_sign[$account['type_sign']]['level']) ? '' : $account_all_type_sign[$account['type_sign']]['level'][$account['level']];
			$account['user_role'] = empty($account_user_roles[$account['uniacid']]['role']) ? '' : $account_user_roles[$account['uniacid']]['role'];
			...
			if (WXAPP_TYPE_SIGN == $account->typeSign) {
				...
				if (!empty($version_info[0]['modules'])) {
					...
					$account['need_upload'] = !empty($last_modules['version']) && $last_modules['version'] < $modules['version'] ? 1 : 0;
				}
			}
	...
	foreach ($history as $key => $item) {
		...
		if (USERS_OPERATE_TYPE_ACCOUNT == $item['type'] && empty($account_info['isdeleted'])) {
			$operate = array(
				...
				'uniacid' => empty($account_info['uniacid']) ? '' : $account_info['uniacid'],
				'type' => empty($account_info['type']) ? '' : $account_info['type'],
				'type_name' => empty($account_info['type_name']) ? '' : $account_info['type_name'],
				'level' => empty($account_info['level']) || empty($account_info['type_sign']) || empty($account_all_type_sign[$account_info['type_sign']]['level']) ? '' : $account_all_type_sign[$account_info['type_sign']]['level'][$account_info['level']],
				'logo' => empty($account_info['logo']) ? '' : $account_info['logo'],
				'switchurl' => empty($account_info['switchurl']) ? '' : $account_info['switchurl'],
				'is_star' => !empty($account_info['is_star']) ? 1 : 0,
			);
...
if ('account_modules' == $do) {
	...
	foreach ($account_modules as $module) {
		...
		$module['is_star'] = !empty($star_info[$module['name']]) ? 1 : 0;
