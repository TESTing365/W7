...
if ('check_scene_str' == $do) {
	$scene_str = safe_gpc_string($_GPC['scene_str']);
...
if ('list' == $do) {
	...
	pdo_query('UPDATE ' . tablename('qrcode') . ' SET status = 0 WHERE uniacid = :uniacid AND model = 1 AND createtime < :createtime - expire', array(':uniacid' => $_W['uniacid'], ':createtime' => $_W['timestamp']));
...
if ('post' == $do) {
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			$update = array(
				...
				'name' => safe_gpc_string($_GPC['scene-name']),
			);
		...
		if (1 == $qrctype) {
			...
			$scene_str = safe_gpc_string($_GPC['scene_str']) ? safe_gpc_string($_GPC['scene_str']) : itoast('场景值不能为空', '', '');
			$is_exist = pdo_fetchcolumn('SELECT id FROM ' . tablename('qrcode') . ' WHERE uniacid = :uniacid AND scene_str = :scene_str AND model = 2', array(':uniacid' => $_W['uniacid'], ':scene_str' => $scene_str));
			if (!empty($is_exist)) {
		...
		if (!is_error($result)) {
			$insert = array(
				...
				'keyword' => $keyword['content'],
				'name' => safe_gpc_string($_GPC['scene-name']),
				'model' => safe_gpc_string($_GPC['qrc-model']),
	...
	if (!empty($_GPC['id'])) {
		$id = intval($_GPC['id']);
		$row = pdo_get('qrcode', array('uniacid' => $_W['uniacid'], 'id' => $id));
...
if ('extend' == $do) {
	$id = intval($_GPC['id']);
	if (!empty($id)) {
		$qrcrow = pdo_get('qrcode', array('uniacid' => $_W['uniacid'], 'id' => $id));
		$update = array();
		if (1 == $qrcrow['model']) {
			$uniacccount = WeAccount::createByUniacid();
