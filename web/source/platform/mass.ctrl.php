...
if ('post' == $do) {
	...
	if (checksubmit('submit')) {
		...
		$group = @json_decode(safe_gpc_html(htmlspecialchars_decode($_GPC['group'], ENT_QUOTES)), true);
		...
		foreach ($_GPC['reply'] as $material_type => $material) {
			...
			if ('reply_basic' == $material_type) {
				$material = safe_gpc_html(htmlspecialchars_decode($material, ENT_QUOTES));
...
if ('preview' == $do) {
	$wxname = safe_gpc_string($_GPC['wxname']);
	if (empty($wxname)) {
		iajax(1, '微信号不能为空', '');
	}
