...
$dos = array('platform', 'system', 'ext', 'account_ext', 'get_fans_kpi', 'get_upgrade_modules', 'get_module_statistics', 'get_not_installed_modules', 'system_home', 'set_top', 'set_default', 'add_welcome', 'ignore_update_module', 'add_welcome_shortcut', 'remove_welcome_shortcut', 'build_account_modules');
...
if ('ext' == $do) {
	...
	if (!empty($_GPC['system_welcome'])) {
		...
		if (empty($_W['current_module']['main_module']) || STATUS_ON == $_GPC['tohome']) {
...
if ('get_fans_kpi' == $do) {
	...
	$yesterday_stat['new'] = empty($yesterday_stat['new']) ? 0 : intval($yesterday_stat['new']);
	$yesterday_stat['cancel'] = empty($yesterday_stat['cancel']) ? 0 : intval($yesterday_stat['cancel']);
	$yesterday_stat['jing_num'] = intval($yesterday_stat['new']) - intval($yesterday_stat['cancel']);
	$yesterday_stat['cumulate'] = empty($yesterday_stat['cumulate']) ? 0 : intval($yesterday_stat['cumulate']);
	//今日粉丝详情
	$today_stat = pdo_get('stat_fans', array('date' => date('Ymd'), 'uniacid' => $_W['uniacid']));
	$today_stat['new'] = empty($today_stat['new']) ? 0 : intval($today_stat['new']);
	$today_stat['cancel'] = empty($today_stat['cancel']) ? 0 : intval($today_stat['cancel']);
