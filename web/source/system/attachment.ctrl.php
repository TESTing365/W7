...
if ('remote' == $do) {
	...
	$remote_urls = array(
		'alioss' => array('old_url' => !empty($remote['alioss']['url']) ? $remote['alioss']['url'] : ''),
		'ftp' => array('old_url' => !empty($remote['ftp']['url']) ? $remote['ftp']['url'] : ''),
		'qiniu' => array('old_url' => !empty($remote['qiniu']['url']) ? $remote['qiniu']['url'] : ''),
		'cos' => array('old_url' => !empty($remote['cos']['url']) ? $remote['cos']['url'] : ''),
	);
	...
	if ($_W['ispost']) {
		$remote = array(
			...
			'ftp' => array(
				...
				'host' => safe_gpc_string($_GPC['ftp']['host']),
				'port' => safe_gpc_string($_GPC['ftp']['port']),
				'username' => safe_gpc_string($_GPC['ftp']['username']),
				'password' => strexists($_GPC['ftp']['password'], '*') ? $_W['setting']['remote_complete_info']['ftp']['password'] : safe_check_password($_GPC['ftp']['password']),
				'pasv' => intval($_GPC['ftp']['pasv']),
				'dir' => safe_gpc_string($_GPC['ftp']['dir']),
				'url' => rtrim(safe_gpc_url($_GPC['ftp']['url'], false), '/'),
			...
			'tcb' => array(
				'url' => strpos($_GPC['tcb']['url'], 'https://') === false ? rtrim(safe_gpc_url('https://' . $_GPC['tcb']['url'], false), '/') : rtrim(safe_gpc_url($_GPC['tcb']['url'], false), '/'),
			),
		);
		switch ($remote['type']) {
			case ATTACH_OSS:
			...
			case ATTACH_FTP:
				...
				attachment_replace_article_remote_url($remote_urls['ftp']['old_url'], $remote['ftp']['url']);
				break;
			case ATTACH_QINIU:
			...
			case ATTACH_COS:
				...
				break;
			case ATTACH_TCB:
				if (empty($remote['tcb']['url'])) {
					iajax(-1, '请填写云托管对象存储安全域名！');
				}
				//上传一个系统图片来验证是否配置好云托管存储
				$_W['setting']['remote']['type'] = ATTACH_TCB;
				$result = file_remote_upload('images/global/nopic.jpg', false);
				if (is_error($result)) {
					iajax(-1, '验证云托管存储时发生错误：' . $result['message']);
				}
				attachment_replace_article_remote_url($remote_urls['tcb']['old_url'], $remote['tcb']['url']);
				break;
		}
		...
		$_W['setting']['remote_complete_info']['tcb'] = $remote['tcb'];
...
if ('ftp' == $do) {
	load()->library('ftp');
	$ftp_config = array(
		'hostname' => safe_gpc_string($_GPC['host']),
		'username' => safe_gpc_string($_GPC['username']),
		'password' => strexists($_GPC['password'], '*') ? $_W['setting']['remote_complete_info']['ftp']['password'] : safe_check_password($_GPC['password']),
		'port' => intval($_GPC['port']),
		'ssl' => safe_gpc_string($_GPC['ssl']),
		'passive' => safe_gpc_string($_GPC['pasv']),
		'timeout' => intval($_GPC['overtime']),
		'rootdir' => safe_gpc_string($_GPC['dir']),
	);
	...
	if (true === $ftp->connect()) {
		// 将上传图片同步到 FTP 服务器
		if ($ftp->upload(IA_ROOT . '/web/resource/images/' . $filename, $filename)) {
