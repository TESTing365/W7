...
if ($_W['isajax'] && $_W['ispost']) {
	$data = array();
	$token = empty($_POST['token']) ? '' : safe_gpc_string(trim($_POST['token']));
	$encodingaeskey = empty($_GPC['encodingaeskey']) ? '' : safe_gpc_string(trim($_GPC['encodingaeskey']));
	$appid = empty($_GPC['appid']) ? '' : safe_gpc_string(trim($_GPC['appid']));
	$appsecret = empty($_GPC['appsecret']) ? '' : safe_gpc_string(trim($_GPC['appsecret']));
	$bindappid = empty($_GPC['bindappid']) ? '' : safe_gpc_string(trim($_GPC['bindappid']));

	$authstate = isset($_GPC['authstate']) ? intval($_GPC['authstate']) : $_W['setting']['platform']['authstate'];
	$data['token'] = isset($_POST['token']) ? $token : $_W['setting']['platform']['token'];
	$data['encodingaeskey'] = isset($_GPC['encodingaeskey']) ? $encodingaeskey : $_W['setting']['platform']['encodingaeskey'];
	$data['appid'] = isset($_GPC['appid']) ? $appid : $_W['setting']['platform']['appid'];
	$data['appsecret'] = isset($_GPC['appsecret']) ? $appsecret : $_W['setting']['platform']['appsecret'];
	$data['bindappid'] = isset($_GPC['bindappid']) ? $bindappid : $_W['setting']['platform']['bindappid'];
