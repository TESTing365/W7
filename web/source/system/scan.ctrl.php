...
if ('post' == $do) {
	...
	if ($_W['ispost'] && $_W['token'] == safe_gpc_string($_GPC['token'])) {
		...
		$info['func'] = safe_gpc_string($_GPC['func']) ? safe_gpc_string($_GPC['func']) : 'com|system|exec|eval|escapeshell|cmd|passthru|base64_decode|gzuncompress';
		$info['code'] = safe_gpc_string($_GPC['code']) ? safe_gpc_string($_GPC['code']) : 'weidongli|sinaapp';
		$info['md5_file'] = empty($_GPC['md5_file']) ? '' : safe_gpc_string($_GPC['md5_file']);
		$info['dir'] = safe_gpc_string($_GPC['dir']);
...
if ('display' == $do) {
	...
	if (empty($badfiles)) {
		if ($_W['isw7_request']) {
			iajax(-1, '扫描结果无异常文件，请选择其他目录');
		}
		itoast('扫描结果无异常文件，请选择其他目录', url('system/scan'), 'error');
	}
