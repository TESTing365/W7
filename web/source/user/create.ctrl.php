...
if ('save' == $do) {
	...
	if (0 == $user_update['groupid']) {
		$user_update['endtime'] = empty($_GPC['timelimit']) ? 0 : strtotime(intval($_GPC['timelimit']) . ' days', TIMESTAMP);
	}
...
if ('check_vice_founder_permission_limit' == $do) {
	if (user_is_vice_founder()) {
		$timelimit['timelimit'] = safe_gpc_string($_GPC['timelimit']);
		$create_account_nums = safe_gpc_array($_GPC['create_account_nums']);
