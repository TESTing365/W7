...
if ('edit_base' == $do) {
	...
	$user['last_visit'] = empty($user['lastvisit']) ? '' : date('Y-m-d H:i:s', $user['lastvisit']);
	$user['joindate'] = empty($user['joindate']) ? '' : date('Y-m-d H:i:s', $user['joindate']);
...
if ('edit_extra_modules' == $do) {
	...
	$templates = empty($_GPC['extra_templates']) ? array() : safe_gpc_array($_GPC['extra_templates']);
	$users_extra_template_table = table('users_extra_templates');
	$users_extra_template_table->deleteExtraTemplatesByUid($uid);
	if (!empty($templates)) {
		foreach ($templates as $template_id) {
