...
if ('save' == $do) {
	...
	$user_group = array(
		...
		'name' => empty($_GPC['name']) ? '' : safe_gpc_string($_GPC['name']),
		'package' => empty($_GPC['package']) ? array() : safe_gpc_array($_GPC['package']),
		'timelimit' => empty($_GPC['timelimit']) ? 0 : safe_gpc_int($_GPC['timelimit']),
		'owner_uid' => 0
	);
	$max_type_all = 0;
	foreach ($account_all_type_sign as $account_type) {
		$maxtype = 'max' . $account_type;
		$user_group[$maxtype] = empty($_GPC[$maxtype]) ? 0 : safe_gpc_int($_GPC[$maxtype]);
		$_GPC[$maxtype] = empty($_GPC[$maxtype]) ? 0 : safe_gpc_int($_GPC[$maxtype]);
...
if ('del' == $do) {
	...
	if (!empty($_W['setting']['register']['groupid']) && $id == $_W['setting']['register']['groupid']) {
