...
$dos = array('display', 'app_display', 'web_display', 'get_account_api', 'get_account_app_api', 'get_account_visit');
...
if ('get_account_api' == $do) {
	$data = array();
	$time_type = safe_gpc_string($_GPC['time_type']);
	...
	if ('web' == $type) {
		...
		if (!empty($visit_info)) {
			foreach ($visit_info as $visit) {
				$all_result['count'][$visit['date']] = empty($all_result['count'][$visit['date']]) ? 0 : $all_result['count'][$visit['date']];
				$all_result['ip_count'][$visit['date']] = empty($all_result['ip_count'][$visit['date']]) ? 0 : $all_result['ip_count'][$visit['date']];
	...
	$result = empty($all_result['count']) ? 0 : $all_result['count'];
	$ip_visit_result = empty($all_result['ip_count']) ? 0 : $all_result['ip_count'];
	if ('today' == $time_type) {
