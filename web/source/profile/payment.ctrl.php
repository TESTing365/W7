...
if ('save_setting' == $do) {
	...
	if ('unionpay' == $type) {
		$unionpay = safe_gpc_array($_GPC['unionpay']);
...
if ('change_status' == $do) {
	...
	$type = in_array($_GPC['type'], $types) ? safe_gpc_string($_GPC['type']) : '';
	if (empty($type)) {
		iajax(-1, '参数错误！');
	}
	$param = safe_gpc_array($_GPC['param']);
	...
	$setting_data = array(
		'pay_switch' => !empty($param['pay_switch']) && 'true' == $param['pay_switch'] ? true : false,
		'recharge_switch' => !empty($param['recharge_switch']) && 'true' == $param['recharge_switch'] ? true : false,
	);
