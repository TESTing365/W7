...
if ('save' == $do || 'test_setting' == $do) {
	...
	$type = empty($_GPC['type']) ? '' : intval($_GPC['type']); //开启的类型
	...
	switch ($op_type) {
		...
		case ATTACH_FTP:
			$op_data = array(
				'hostname' => !empty($post['host']) ? $post['host'] : $post['hostname'],
				'port' => empty($post['port']) ? 21 : $post['port'],
				'ssl' => intval($post['ssl']),
				'username' => $post['username'],
				'password' => strexists($post['password'], '*') ? $remote['ftp']['password'] : $post['password'],
				'passive' => intval(!empty($post['pasv']) ? $post['pasv'] : $post['passive']) ,
				'rootdir' => !empty($post['dir']) ? $post['dir'] : $post['rootdir'],
				'timeout' => intval(!empty($post['overtime']) ? $post['overtime'] : $post['timeout']),
				'url' => trim(trim($post['url']), '/')
			);
	...
	if (!empty($test_type)) {
		...
		$response = ihttp_request($op_data['url'] . '/favicon.ico', array(), array('CURLOPT_REFERER' => $_SERVER['SERVER_NAME']));
