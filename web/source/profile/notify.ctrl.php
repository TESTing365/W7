...
if ('mail' == $do) {
	...
	if (checksubmit('submit')) {
		$notify['mail'] = array(
			...
			'smtp' => safe_gpc_array($_GPC['smtp']),
			'sender' => safe_gpc_string($_GPC['sender']),
			'signature' => safe_gpc_string($_GPC['signature']),
		);
		if (is_error($notify['mail']['password'])) {
			itoast($notify['mail']['password']['message'], '', 'error');
		}
		if (1 == intval($_GPC['status'])) {
